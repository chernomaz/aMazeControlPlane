#!/usr/bin/env bash
# Sprint 8 — Phase 8B: NEMO substrate extensions.
#
# Boots the Phase 8A stack plus the Phase 8B additions (currently: MCP-server
# NEMO container), waits for health, runs the 8A regression suite and the
# 8B system tests in sequence against the real stack (no mocks).
#
#   Usage:
...
echo "=== Sprint 8 — Phase 8B: NEMO substrate extensions ==="
...
echo "[3/4] Running Phase 8A regression"
set +e
/home/ubuntu/venv/bin/python "$REPO/tests/system_test_sprint8a.py"
rc_a=$?
set -e

echo "[4/4] Running Phase 8B system tests"
set +e
...
if [ $rc_a -ne 0 ] || [ $rc_b -ne 0 ]; then
  echo ""
  echo "[sprint8b] tests failed (8A=$rc_a, 8B=$rc_b) — compose logs tail:"
