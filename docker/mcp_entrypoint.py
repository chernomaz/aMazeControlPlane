"""MCP-server container entrypoint — Phase 8B Slice 1.
...
def fail_fatal(msg: str) -> None:
    ...
    """
    print(f"[mcp {MCP_ID}] FATAL: {msg}", flush=True)
    os._exit(1)
    ...
    for attempt in range(30):
        try:
            ...
            with urllib.request.urlopen(req, timeout=3) as resp:
                ...
                print(f"[mcp {MCP_ID}] registered: {payload}", flush=True)
                return
        except urllib.error.HTTPError as e:
            if 400 <= e.code < 500:
                fail_fatal(f"register rejected {e.code} — payload bad")
            print(
                f"[mcp {MCP_ID}] register attempt {attempt+1} got {e.code}",
                flush=True,
            )
            time.sleep(2)
        except (urllib.error.URLError, ConnectionError) as e:
            print(
                f"[mcp {MCP_ID}] register attempt {attempt+1} failed: {e}",
                flush=True,
            )
            time.sleep(2)
...
def run_server() -> None:
    ...
    print(
        f"[mcp {MCP_ID}] starting mcp on 0.0.0.0:{MCP_PORT}",
        flush=True,
    )
